...
int list(string what)
{
  ...
  else if (display[0]=="Invalid command")
  {
    ...
    write(toappend);
// The linux terminal has 80 characters. This character length is divided to 3 columns. 
// The first two columns having char length of 27 and the last one having char length 26
  int i = 0;
  foreach(display,string str)
  {
    if(i==2)
    {
      write(sprintf("%-26s",str));
      i=0;
    }
    else
    {
      write(sprintf("%-27s",str));
      i++;
    }
  }
 }
